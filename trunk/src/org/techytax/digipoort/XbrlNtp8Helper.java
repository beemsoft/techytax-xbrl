...
public class XbrlNtp8Helper {
	...
	public synchronized static String createXbrlInstance(
		...
		try {
			...
			simpleType
					.setHref("http://www.nltaxonomie.nl/8.0/report/bd/entrypoints/bd-rpt-ob-aangifte-2014.xsd");
			...
			period.setStartDate(DateHelper.getDate(vatDeclarationData
					.getStartDate()));
			period.setEndDate(DateHelper.getDate(vatDeclarationData
					.getEndDate()));
			...
			xbrl.getItemOrTupleOrContext().add(
					bdItemObjectFactory.createContactType(contactType));

			Anstring10VItemType initials = bdTypeObjectFactory
					.createAnstring10VItemType();
			initials.setValue(vatDeclarationData.getUser().getInitials());
			initials.setContextRef(context);
			xbrl.getItemOrTupleOrContext().add(
					bdAlgemeenObjectFactory.createContactInitials(initials));

			Anstring10VItemType prefix = bdTypeObjectFactory
					.createAnstring10VItemType();
			prefix.setValue(vatDeclarationData.getUser().getPrefix());
			prefix.setContextRef(context);
			xbrl.getItemOrTupleOrContext().add(
					bdAlgemeenObjectFactory.createContactPrefix(prefix));

			Anstring200VItemType surname = bdTypeObjectFactory
					.createAnstring200VItemType();
			surname.setValue(vatDeclarationData.getUser().getSurname());
			surname.setContextRef(context);
			xbrl.getItemOrTupleOrContext().add(
					bdAlgemeenObjectFactory.createContactSurname(surname));

			Anstring14VItemType telephoneNumber = bdTypeObjectFactory
					.createAnstring14VItemType();
			telephoneNumber.setValue(vatDeclarationData.getUser()
					.getPhoneNumber());
			telephoneNumber.setContextRef(context);
			xbrl.getItemOrTupleOrContext().add(
					bdAlgemeenObjectFactory
							.createContactTelephoneNumber(telephoneNumber));
			...
			xbrl.getItemOrTupleOrContext().add(
					bdAlgemeenObjectFactory.createDateTimeCreation(dateTime));

			MessageReferenceSupplierVATItemType supplier = bdTypeObjectFactory
					.createMessageReferenceSupplierVATItemType();
			supplier.setValue(Integer.toString(messageReference++));
			supplier.setContextRef(context);
			xbrl.getItemOrTupleOrContext().add(
					bdAlgemeenObjectFactory
							.createMessageReferenceSupplierVAT(supplier));

			Anstring20VItemType packageVersion = bdTypeObjectFactory
					.createAnstring20VItemType();
			packageVersion.setValue(PACKAGE_VERSION);
			packageVersion.setContextRef(context);
			xbrl.getItemOrTupleOrContext().add(
					bdAlgemeenObjectFactory
							.createSoftwarePackageVersion(packageVersion));

			Anstring30VItemType packageName = bdTypeObjectFactory
					.createAnstring30VItemType();
			packageName.setValue(PACKAGE_NAME);
			packageName.setContextRef(context);
			xbrl.getItemOrTupleOrContext().add(
					bdAlgemeenObjectFactory
							.createSoftwarePackageName(packageName));

			String softwareVendorAccountNumber = PropsFactory
					.getProperty("software.vendor.account.number");
			Anstring8FItemType softwareVendor = bdTypeObjectFactory
					.createAnstring8FItemType();
			softwareVendor.setValue(softwareVendorAccountNumber);
			softwareVendor.setContextRef(context);
			xbrl.getItemOrTupleOrContext().add(
					bdAlgemeenObjectFactory
							.createSoftwareVendorAccountNumber(softwareVendor));
			...
			valueAddedTaxOnInput.setValue(vatDeclarationData
					.getValueAddedTaxOnInput());
			xbrl.getItemOrTupleOrContext().add(
					bdItemObjectFactory
							.createValueAddedTaxOnInput(valueAddedTaxOnInput));
			...
			valueAddedTaxOwed.setValue(vatDeclarationData
					.getValueAddedTaxOwed());
			xbrl.getItemOrTupleOrContext().add(
					bdItemObjectFactory
							.createValueAddedTaxOwed(valueAddedTaxOwed));
			...
			valueAddedTaxOwedToBePaidBack.setValue(vatDeclarationData
					.getValueAddedTaxOwedToBePaidBack());
			xbrl.getItemOrTupleOrContext()
					.add(bdItemObjectFactory
							.createValueAddedTaxOwedToBePaidBack(valueAddedTaxOwedToBePaidBack));
			...
			valueAddedTaxPrivateUse.setValue(vatDeclarationData
					.getValueAddedTaxPrivateUse());
			xbrl.getItemOrTupleOrContext()
					.add(bdItemObjectFactory
							.createValueAddedTaxPrivateUse(valueAddedTaxPrivateUse));
	...
	public static void addBalanceData(VatDeclarationData vatDeclarationData,
			Balans balans) {
		BigInteger totaleKosten = AmountHelper.roundToInteger(balans
				.getTotaleKosten());
		BigInteger correction = AmountHelper.roundToInteger(balans
				.getCorrection());
		BigInteger totaleBaten = AmountHelper.roundToInteger(balans
				.getTotaleBaten());
		BigInteger owed = totaleBaten.subtract(totaleKosten).subtract(
				correction);
		vatDeclarationData.setValueAddedTaxOwed(owed);
		vatDeclarationData
				.setValueAddedTaxOnInput(totaleKosten.add(correction));
		vatDeclarationData.setValueAddedTaxOwedToBePaidBack(owed);
		vatDeclarationData.setValueAddedTaxPrivateUse(correction);
		vatDeclarationData
				.setValueAddedTaxSuppliesServicesGeneralTariff(totaleBaten);
	}
